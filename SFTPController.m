...
- ( IBAction )cancelConnection: ( id )sender
{
  ...
  if ( [ uploadQueue count ] ) {
    rc = NSRunAlertPanel( NSLocalizedString( @"Warning: Upload queue not empty.",
                         ...
                         NSLocalizedString( @"There are %lu items left in the "
                                           ...
                                           @"you want to disconnect without uploading all of them?" ),
                         NSLocalizedString( @"Disconnect", @"Disconnect" ),
                         NSLocalizedString( @"Cancel", @"Cancel" ), @"", (unsigned long)[ uploadQueue count ] );
...
- ( void )requestPasswordWithPrompt: ( char * )header
{
  ...
  if ( ! [ self firstPasswordPrompt ] && [ self gotPasswordFromKeychain ] ) {
    ...
    NSString *msgFormat = NSLocalizedString( @"The password for this server in your "
                                            ...
                                            @"keychain does not seem to be correct. Would "
                                            @"you like to open Keychain Access to edit it?" );
    NSString *defaultButton = NSLocalizedString( @"Open Keychain Access", @"Open Keychain Access" );
    NSString *alternateButton = NSLocalizedString( @"Cancel", @"Cancel" );
    
    
    rc = NSRunAlertPanel( title,
                         @"%@", msgFormat,
                         defaultButton,
                         alternateButton,
                         @"" );
    ...
    switch ( rc ) {
      case NSAlertDefaultReturn:
        if ( ! [[ NSWorkspace sharedWorkspace ] launchApplication: @"Keychain Access" ] ) {
          NSRunAlertPanel( NSLocalizedString( @"Error", @"Error" ),
...
- ( BOOL )handleEvent: ( NSEvent * )theEvent fromTable: ( SFTPTableView * )table
{
  ...
  if ( key == NSDeleteCharacter ) {
    ...
    e = [ NSEvent mouseEventWithType: NSEventTypeRightMouseUp location: p
